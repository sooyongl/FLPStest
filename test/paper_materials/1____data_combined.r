...
#   group_by(lvmodel, samplesize, nitem, .add = T) %>%
#   group_split() %>%
#   map_df( ~ .x %>%
#             mutate(rep_type = c(rep("initial", 100),rep("initial", nrow(.x)-100))
#                    )) %>%
#   select(lvmodel, samplesize, nitem, rep, rep_type)
...
# mpart_add <- readRDS(paste0("test/shinydata/",data_idx,"_mpart.rds")) %>%
#   rename(est_mean = mean)
# total_res_add <- readRDS(paste0("test/shinydata/",data_idx,"_data.rds"))
#
...
# total_res2 %>%
#   ungroup() %>%
#   distinct(lvmodel, samplesize, nitem) %>%
#   count(lvmodel, samplesize, nitem) %>%
#   print(n = 100)

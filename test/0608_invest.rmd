...
runtime: shiny
resource_files:
- shinydata/0608_data.rds
- shinydata/0608_mpart.rds
---
...
total_res <- readRDS("shinydata/0608_data.rds")
mpart <- readRDS("shinydata/0608_mpart.rds") %>% 
...
renderPlot({
  ...
  mpart_sub <- mpart %>%
    ...
    mutate(
      ...
      X2.5err = X2.5. - true_param,
      X97.5err = X97.5. - true_param,
    )
  
  covarage <- mpart_sub %>%
    group_by(cond, par_name0) %>%
    summarise(coverage = mean(CIin))
  ...
  peta <-
    ...
    geom_point() +
    geom_errorbar(aes(ymin=X2.5err, ymax=X97.5err), 
                  width=.2, alpha = 0.5) +
    geom_hline(yintercept = 0, 
               color = "red", 
    ...
    annotate(geom = "text",
             ...
             vjust = 2,
             hjust = 0,
             label = paste0("Covarage = ", round(covarage$coverage[1],3))) +
    labs(y = "Eta") +
  ...
  if(nrow(covarage) == 2) {
    ...
    ptau <- mpart_sub %>%
      ...
      annotate(geom = "text",
               ...
               vjust = 2,
               hjust = 0,
               label = paste0("Covarage = ", round(covarage$coverage[2],3))) +
      labs(y = "Tau") +
      theme(axis.text.x = element_blank())
    
    
    ggpubr::ggarrange(peta, ptau,
                      nrow = 2,
                      labels = c("Eta","Tau"))
    ...
    ptau <- mpart_sub %>%
      ...
      annotate(geom = "text",
               ...
               vjust = 2,
               hjust = 0,
               label = paste0("Covarage = ", round(covarage$coverage[3],3))) +
      labs(y = "Tau") +
      theme(axis.text.x = element_blank())

...
for(i in 1:10) {
  ...
  dt <- do.call("makeDat", parsFromMod)
  dt$nclass <- 2
  ...
  #                     collapse = "\n")
  stan_model <- paste(read_lines("test/test_lv_model/stan/LCA_indprop_test.stan"),
                      collapse = "\n")
  # cat(stan_model)
  lca.fit <- rstan::stan(
  ...
  saveRDS(res, paste0("results/res_indiprop_", "LCA","_",i ,".rds"))

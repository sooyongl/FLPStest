...
for(i in 1:length(files))
{
  ...
  pop_pars <- fit$pop_pars
  pop_data <- fit$pop_data
  ...
  gel <- FLPS:::my.gelman.diag(mcmc_object)
  gel <- gel$psrf[1]
  ...
  if(tolower(pop_pars$lvmodel) == "sem") {

    diff <- list(
      list(
        est = colMeans(fit[str_detect(names(fit), "lambda")]),
        pop = pop_data$lv.par[,1]
      )
    )
  }

  if(tolower(pop_pars$lvmodel) == "rasch") {
  ...
  if(tolower(pop_pars$lvmodel) == "2pl") {

    diff <- list(
      list(
        est.b = colMeans(fit[str_detect(names(fit), "beta\\[")]),
        pop.b = pop_data$lv.par$b,

        est.a = colMeans(fit[str_detect(names(fit), "alpha\\[")]),
        pop.a = pop_data$lv.par$a
      )
    )
  }

  if(tolower(pop_pars$lvmodel) == "gpcm") {

    diff <- list(
      list(
        est.b1 = colMeans(fit[str_detect(names(fit), "beta1\\[")]),
        pop.b1 = pop_data$lv.par$b.1,

        est.b2 = colMeans(fit[str_detect(names(fit), "beta2\\[")]),
        pop.b2 = pop_data$lv.par$b.2,

        est.b3 = colMeans(fit[str_detect(names(fit), "beta3\\[")]),
        pop.b3 = pop_data$lv.par$b.3,
...
# if(!file.exists("report/rds/raw_results_1101.rds")){
  saveRDS(res1, "report/rds/raw_results_1111_1.rds")
  # }

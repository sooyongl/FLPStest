...
model{
 ...
 matrix[nfac, nfac] A0;  
 vector[nfac] ggmean; 
 //array[nstud] vector[nfac] ggmean;
 ...
 for(i in 1:nstud){
  ...
  useEff[i]= to_row_vector(eta[i, ])*a1;

  //trtEff[i]=b0 + b11*eta[i, 1] + b12*eta[i, 2];
  trtEff[i] = b0 + to_row_vector(eta[i, ])*b1;
  ...
  //for (n in 1:N) {
  //  ggmean[n] = beta000 + beta * x[n];
  //}
  
 //ggmean[1] = b111 + to_matrix(X*betaU1)[1,1];
 //ggmean[2] = b222 + to_matrix(X*betaU2)[1,1];
 for(ifac in 1:nfac) {
   ggmean[ifac] = gm[ifac] + mean(X*col(betaU, ifac));
 };
